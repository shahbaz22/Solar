'not working porgram for a1>0,a2>0,<0 for K with surface plot'
import numpy as np
import scipy
...
al1=np.linspace(1,3,400) # Bessel functions only take positive values
al2=np.linspace(1,3,400) #Assures that the arrays have the same number of elements
...
def kn(alpha1,alpha2):
    ...
    'for a2>0'
    if alpha2=0 and alpha1=0:
        kn=0
    if alpha2>0:
        b2prod=-j(1,a1)*y(0,a2)-j(0,a1)*y(1,a2)
        c2b2rat=j(0,a1)*j(1,a2)+j(1,a1)*j(0,a2)
...
norm = cc.LogNorm(vmin=0.1, vmax=10)
#norm = cc.Normalize(vmin=0.1, vmax=10)
surf=ax.plot_surface(alpha1,alpha2, fn_mod, cmap=cm.hot,
